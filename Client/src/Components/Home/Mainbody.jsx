...
const Body = () => {
 ...
 <section ref={MetaAppRef} className="mx-auto max-w-5xl px-4 py-16 sm:px-8 sm:py-20">
  <div className="text-center">
    <span className="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-600">Get Started</span>
    <h2 className="mt-2 text-2xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">
      Choose Your Trading Platform
    </h2>
    <p className="mt-3 text-gray-500">
      Download the official MetaTrader installer to run your EA and start trading.
    </p>
  </div>

  <div
    className={`mt-12 grid gap-6 transition-all duration-700 ease-out sm:grid-cols-2 ${
      scrolled.footerScroll ? 'translate-y-0 opacity-100' : 'translate-y-6 opacity-0'
    }`}
  >
    {platforms.map((platform, index) => (
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="group relative flex flex-col items-center overflow-hidden rounded-2xl border border-gray-100 bg-white p-8 text-center shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-lime-200 hover:shadow-xl"
      >
        <span className="absolute right-4 top-4 rounded-full bg-emerald-50 px-3 py-1 text-[10px] font-semibold uppercase tracking-wide text-emerald-600">
          Free
        </span>

        <div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-gray-50 transition-colors group-hover:bg-lime-50">
          <img src={platform.img} alt={platform.name} className="h-12 w-auto" />
        </div>

        <h4 className="mt-5 text-xl font-bold text-gray-900">{platform.name}</h4>
        <p className="mt-1 text-sm text-gray-500">{platform.description}</p>

        <span className="mt-5 inline-flex items-center gap-1.5 rounded-full bg-black px-5 py-2 text-sm font-semibold text-lime-400 transition-colors group-hover:bg-gray-900">
          <RiDownloadLine />
          Download
        </span>
      </a>
